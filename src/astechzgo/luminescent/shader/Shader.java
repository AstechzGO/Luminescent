...
public class Shader {
	...
	private int loadAndCompileShader(String source, int shaderType) {
		...
		if( handle == 0 )
		{
			throw new RuntimeException("Could not created shader of type "+shaderType+" for shader "+this.getName());
		}
		...
		{
			throw new IllegalStateException("Compilation error for shader ["+this.getName()+"]. Reason: " + GL20.glGetShaderInfoLog(handle, 1000));
		}
 
		System.out.println(GL20.glGetShaderInfoLog(handle, 1000));
